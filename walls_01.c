...
void	calc_step_and_side_dist(t_data *g, t_wall *w)
{
	...
	{
		w->step_x = -1;
		w->side_dist_x = (g->player_pos_x - w->map_x) * w->delta_dist_x;
	}
	else
	{
		w->step_x = 1;
		w->side_dist_x = (w->map_x + 1.0 - g->player_pos_x) * w->delta_dist_x;
	}
	if (w->ray_dir_y < 0)
	{
		w->step_y = -1;
		w->side_dist_y = (g->player_pos_y - w->map_y) * w->delta_dist_y;
	}
	else
	{
		w->step_y = 1;
		w->side_dist_y = (w->map_y + 1.0 - g->player_pos_y) * w->delta_dist_y;
	}
}
...
void	check_position(t_data *g, t_wall *w)
{
	...
	w->ray_dir_x = g->player_dir_x + g->cam_plane_x * w->camera_x;
	w->ray_dir_y = g->player_dir_y + g->cam_plane_y * w->camera_x;
	//в какой части карты мы находимся
	w->map_x = (int)g->player_pos_x;
	w->map_y = (int)g->player_pos_y;
...
void	calc_size_of_wall_x(t_data *g, t_wall *w)
{
	...
	if (w->side == 0)
		w->perp_wall_dist = (w->map_x - g->player_pos_x + (1 - w->step_x) / 2) / w->ray_dir_x;
	else
		w->perp_wall_dist = (w->map_y - g->player_pos_y + (1 - w->step_y) / 2) / w->ray_dir_y;
